...
def mask_prompt(input_ids: list[int], n_prompt: int) -> list[int]:
    """labels для лосса: -100 на промпте, в лосс идёт только ответ ассистента."""
...
def truncation_stats(metas: list[dict], name: str, params: dict) -> dict:
    ...
    if ratio > warn:
        print(
            f"  ВНИМАНИЕ {name}: обрезано {truncated} из {total} ({ratio:.1%}) "
            f"при пороге {warn:.1%} — поднимите tokenize.max_seq_len"
        )
