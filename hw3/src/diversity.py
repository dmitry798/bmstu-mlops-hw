...
def main() -> None:
    ...
    if failed:
        ...
        # не мешает вырожденному набору доехать до обучения: пайплайн зелёный,
        # предупреждение теряется в логе, модель учит шаблон. Поэтому —
        # падение с перечислением всех нарушенных порогов сразу (а не первого:
        # чинить по одному порогу за запуск — это N прогонов пайплайна).
        # Метрики уже записаны выше: разбираться есть по чему.
        raise DiversityError(
            f"{paths['clean']}: набор вырожден, нарушено порогов {len(failed)}:\n  - "
            + "\n  - ".join(failed)
            + "\n\nПороги живут в params.yaml → diversity. Менять их можно, "
              "но с обоснованием в docs/datasheet.md."
        )
...
if __name__ == "__main__":
    # Исключение — для вызова из кода, а читателю лога нужен текст, а не
    # traceback: стадия падает, но сообщение должно объяснять, что чинить.
    try:
        main()
