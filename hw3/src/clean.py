...
def main() -> None:
    ...
    # 5. Near-dup дедупликация по тому же ключу.
    # Точный хэш ловит только побайтовые повторы. Пара «тот же вопрос,
    # переставленные варианты / другие пробелы» для него — две разные строки,
    # и без этого шага она доезжает до обучения и до сплита (а оттуда —
    # в контаминацию train/test). Порядок «сначала точная, потом MinHash»
    # не косметический: дешёвая операция снимает основную массу, дорогой
    # MinHash работает по остатку.
    near: set[int] = set()
